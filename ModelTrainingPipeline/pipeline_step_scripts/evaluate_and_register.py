...
# registry. For the clustering approach we will register by default. 

# Load Clustering  Model
model = joblib.load(os.path.join(training_step_pipeline_data, 'model.pkl'))
updated_tags = model.get_params()
updated_tags['Inertia'] = model.inertia_

model = current_run.register_model(model_name, model_path='model_files', description=model_description, model_framework='tslearn', model_framework_version=tslearn.__version__, tags=updated_tags, datasets=formatted_datasets, sample_input_dataset = training_dataset)
